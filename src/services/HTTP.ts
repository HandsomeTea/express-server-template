...
import type { AxiosRequestConfig, InternalAxiosRequestConfig, AxiosResponse, AxiosError, Method as AxiosMethod, ResponseType, AxiosInstance } from 'axios';
import Agent from 'agentkeepalive';

import { getContext, log, generateTraceId } from '@/configs/index.js';
// import JWT from './JWT';
	...
	beforeSendToServer(config: InternalAxiosRequestConfig) {
		if (config.baseURL && systemService.has(config.baseURL)) {
			...
			if (!config.headers['x-b3-traceid']) {
				config.headers['x-b3-traceid'] = ctx?.traceId || generateTraceId();
			}

			if (!config.headers['x-b3-parentspanid']) {
				config.headers['x-b3-parentspanid'] = ctx?.parentSpanId || '';
			}
		}
